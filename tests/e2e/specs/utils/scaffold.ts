...
export async function verifyGeneratedProject(
    ...
    await expect(envDocument.editorContent).toContainText(new RegExp(`CC_BOOTSTRAP_SERVER\\s*=\\s*"?${bootstrapServers}"`));
    if (topic) {
        await expect(envDocument.editorContent).toContainText(new RegExp(`CC_TOPIC\\s*=\\s*"?${topic}"`));
    }
    // and we should see the client.id starting with the expected prefix
    await expect(envDocument.editorContent).toContainText(new RegExp(`CLIENT_ID\\s*=\\s*"vscode-${templateName}`));
}
