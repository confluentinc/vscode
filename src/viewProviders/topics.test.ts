...
describe("TopicViewProvider methods", () => {
  ...
  it("Test isFocusedOnCCloud() should return true when the cluster is a CCloud one", () => {
    provider.kafkaCluster = TEST_CCLOUD_KAFKA_CLUSTER;
    assert.strictEqual(provider.isFocusedOnCCloud(), true);
  });

  it("Test isFocusedOnCCloud() should return false when the cluster is not a CCloud one", () => {
    provider.kafkaCluster = TEST_LOCAL_KAFKA_CLUSTER;
    assert.strictEqual(provider.isFocusedOnCCloud(), false);
  });

  it("Test isFocusedOnCCloud() should return false when the cluster is null", () => {
