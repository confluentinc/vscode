...
describe("SchemasViewProvider search behavior", () => {
  ...
  it("Test isFocusedOnCCloud() should return true when the current schema registry is a CCloud one", () => {
    const isFocused = provider.isFocusedOnCCloud();
    assert.strictEqual(isFocused, true);
  });

  it("Test isFocusedOnCCloud() should return false when the current schema registry is not ccloud", () => {
    provider.schemaRegistry = TEST_LOCAL_SCHEMA_REGISTRY;
    const isFocused = provider.isFocusedOnCCloud();
    assert.strictEqual(isFocused, false);
  });

  it("Test isFocusedOnCCloud() should return false when the current schema registry is null", () => {
