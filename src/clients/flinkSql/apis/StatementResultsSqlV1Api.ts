...
export class StatementResultsSqlV1Api extends runtime.BaseAPI {
     ...
     */
    async getSqlv1StatementResultRaw(requestParameters: GetSqlv1StatementResultRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<GetSqlv1StatementResult200Response>> {
        ...
        const response = await this.request({
            path: `/sql/v1/organizations/{organization_id}/environments/{environment_id}/statements/{name}/results`.replace(`{${"organization_id"}}`, encodeURIComponent(String(requestParameters['organization_id']))).replace(`{${"environment_id"}}`, encodeURIComponent(String(requestParameters['environment_id']))).replace(`{${"name"}}`, encodeURIComponent(String(requestParameters['name']))),
            method: 'GET',
            headers: headerParameters,
            query: queryParameters,
        }, initOverrides);

        return new runtime.JSONApiResponse(response, (jsonValue) => GetSqlv1StatementResult200ResponseFromJSON(jsonValue));
    }
     ...
     * Read Statement Result
     */
    async getSqlv1StatementResult(requestParameters: GetSqlv1StatementResultRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<GetSqlv1StatementResult200Response> {
