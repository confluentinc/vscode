...
/** Return the file URI for the sidecar's formatted log file, normalized for the user's OS. */
export function sidecarFormattedLogFileUri(): Uri {
  return Uri.file(normalize(getSidecarFormattedLogfilePath()));
}
...
async function saveSidecarLogFile() {
  ...
  const formatChoice = await window.showQuickPick(
    [
      ...
      },
      {
        ...
        description: "Original JSON log format",
        detail: "Raw logs as generated by the sidecar process",
        id: "raw",
      },
    ],
    {
  ...
  const isFormatted = formatChoice.id === "formatted";
  const defaultFileName = isFormatted
    ? "vscode-confluent-sidecar-formatted.log"
    : "vscode-confluent-sidecar.log";

  // prompt where to save the file, using the home directory as the default
  const defaultPath = join(homedir(), defaultFileName);
  const saveUri: Uri | undefined = await window.showSaveDialog({
    defaultUri: Uri.file(defaultPath),
    filters: {
      "Log files": ["log"],
    },
    title: `Save Confluent Extension Sidecar Log (${formatChoice.label})`,
  });

  if (saveUri) {
    const sourceUri = isFormatted ? sidecarFormattedLogFileUri() : sidecarLogFileUri();
    await handleLogFileSave(sourceUri, saveUri, true);
  }
}
 ...
 * - "Confluent: Sidecar" output channel log file (both raw JSON and formatted versions)
...
async function saveSupportZip() {
  ...
  // then the formatted log file, if it exists
  try {
    fileEntries.push({
      sourceUri: sidecarFormattedLogFileUri(),
      zipPath: "vscode-confluent-sidecar-formatted.log",
    });
  } catch {
    // shouldn't happen, but if the formatted log file doesn't exist yet, we'll log and move on
    logger.debug("Formatted sidecar log file not found, skipping in support zip");
  }
