...
describe("CCloudAuthStatusMiddleware behavior", () => {
  ...
  it("pre() should call handleCCloudAuthStatus() when the correct CCloud connection header is passed", async () => {
  ...
  it("pre() should not call handleCCloudAuthStatus() when the CCloud connection header is missing", async () => {
  ...
  it("pre() should call handleCCloudInvalidTokenStatus() from an INVALID_TOKEN auth status", async () => {
    const requestContext = fakeRequestWithHeader(
      SIDECAR_CONNECTION_ID_HEADER,
      CCLOUD_CONNECTION_ID,
    );
  ...
  it("pre() should fire ccloudAuthSessionInvalidated from a NO_TOKEN or FAILED auth status", async () => {
    const requestContext = fakeRequestWithHeader(
      SIDECAR_CONNECTION_ID_HEADER,
      CCLOUD_CONNECTION_ID,
    );
  ...
  it("pre() should not block requests from a VALID_TOKEN auth status", async () => {
    const requestContext = fakeRequestWithHeader(
      SIDECAR_CONNECTION_ID_HEADER,
      CCLOUD_CONNECTION_ID,
    );
